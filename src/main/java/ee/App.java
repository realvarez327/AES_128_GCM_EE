...
//        For alt choice, fix issue in multiplicative inverse
//
//        byte intermediate = 0b00;
//        if (inputByte != 0b00) {
//            intermediate = findMultiplicativeInverseRecursive(inputByte);
//        }
//        int position = 0;
//        while (position < 8) {
//            int current = (intermediate >> position) & 1;
//            current = current ^ (intermediate >> ((position + 4) % 8) & 1) ^ ((intermediate >> ((position + 5) % 8)) & 1) ^ ((intermediate >> ((position + 6) % 8)) & 1) ^ ((intermediate >> ((position + 7) % 8)) & 1);
//            if (current == 1) {
//                intermediate = (byte) (intermediate | (1 << intermediate));
//            } else {
//                intermediate = (byte) (intermediate & ~(1 << position));
//            }
//            position++;
//        }
//
